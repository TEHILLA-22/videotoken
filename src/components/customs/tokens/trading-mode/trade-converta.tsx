...
export default function TradingControls({
  ...
  const connection = new Connection(
    'https://mainnet.helius-rpc.com/?api-key=77aae9b3-ad37-4523-8caf-dea409d5519e',
  );
  ...
  const handleExecuteTrade = async () => {
    try {
      ...
      // 1️⃣ Execute on-chain swap
      const result = await performSwap({
        action: type, // buy | sell
        mint: tokenMint,
        amount, // string
        publicKey: publicKey.toBase58(),
        denominatedInSol: type === 'buy',
      ...
      // 3️⃣ Use the correct field name `signature`
      const txSignature = result.signature;

      if (!txSignature) {
        setError('Transaction succeeded but no tx signature returned');
        return;
      }

      // 4️⃣ Save order to MongoDB
      ...
      if (!saveRes.ok) {
        console.warn('Order saved failed, but swap succeeded');
      }

      // 5️⃣ Success UI
      toast.success('✅ Trade executed & saved');
      setAmount('');
      setLimitPrice('');
    } catch (err: any) {
      console.error(err);
      setError('Error during Pump trade execution');
    } finally {
  ...
  return (
    <div className="flex flex-col max-w-full md:max-w-xl lg:max-w-full w-full relative z-10 top-0 sm:p-4 xl:max-w-md">
        ...
        <TabsList className="grid grid-cols-2 w-full h-auto py-2 gap-2">
          <TabsTrigger
            ...
            className={cn(
              ...
              type === 'buy' &&
                'data-[state=active]:bg-[#00BC45]/50 border border-[#00BC45] data-[state=active]:shadow-none',
            )}
          >
            Buy
          </TabsTrigger>
      ...
      <div className="flex justify-between text-sm text-zinc-400 mb-3 px-1">
        ...
        <div className="flex items-center gap-1.5 text-white">
          <span>{solbalance !== null ? `${solbalance.toFixed(4)} SOL` : '—'}</span>
          <SiSolana className="h-3.5 w-3.5" />
        </div>
      </div>

      {error && <div className="text-rose-500 text-sm mb-3 px-1">{error}</div>}
      <Button
